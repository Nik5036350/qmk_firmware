...
[_QWERTY] = LAYOUT_planck_grid(
    ...
    MT(MOD_LSFT, KC_CLCK), KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,     KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_ENT ,
...
[_LOWER] = LAYOUT_planck_grid(
    ...
    _______, KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,  S(KC_NUHS), S(KC_NUBS), KC_HOME, KC_PGDN, _______,
...
void matrix_scan_user(void) {
  LEADER_DICTIONARY() {
        ...
        SEQ_TWO_KEYS(KC_G, KC_L) {  // git log
            SEND_STRING("git log --oneline --graph");
        }

        // SEQ_TWO_KEYS(KC_G, KC_P) {  // git push upstream
        //     SEND_STRING("git push origin $(git rev-parse --abbrev-ref HEAD)");
        // }

        // SEQ_THREE_KEYS(KC_G, KC_F, KC_P) {  // git fetch and pull
        //   SEND_STRING("git fetch && git pull origin $(git rev-parse --abbrev-ref HEAD)");
        // }j

        SEQ_THREE_KEYS(KC_G, KC_C, KC_A) {  // select all and copy
            SEND_STRING("git commit --ammend");
            tap(KC_ENT);
        }
    }
  #ifdef AUDIO_ENABLE
